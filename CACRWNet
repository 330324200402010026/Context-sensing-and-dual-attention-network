...
class ResNet_New(nn.Module):
    ...
    def __init__(self,
        ...
        # 替换原有的SE模块为分组注意力
        self.group_att = GroupedChannelAttention(channel=256, groups=4)
...
#
        ...
        U4 = self.SEFPN3(U3, d2)
        U4 = self.spatial_att(U4) * U4  # 添加空间注意力

#
# # ####################### V型网络 #######################
        #应用分组注意力
        d2 = self.RSU1(d2)
        d2 = self.group_att(d2)  # 替换原有的SE模块
        d2 = U4
        d3 = U3
        d4 = U2
        d5 = U1
...
# # #  #################### 注意力机制 #######################
        ...
        d2 = self.group_att(d2)
        d3 = self.group_att(d3)
        d4 = self.group_att(d4)
        d5 = self.group_att(d5)
