...
class GroupedChannelAttention(nn.Module):
    """分组通道注意力模块"""
    def __init__(self, channel, groups=4, reduction=16):
        super().__init__()
        self.groups = groups
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.fc = nn.Sequential(
            nn.Conv2d(channel, channel//reduction, 1, groups=groups, bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(channel//reduction, channel, 1, groups=groups, bias=False),
            nn.Sigmoid()
        )
